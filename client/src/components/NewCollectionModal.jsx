...
export default function NewCollectionModal({ isOpen, onClose, onCreated }) {
  ...
  const createIt = async () => {
    ...
    setErr("");
    try {
      await dispatch(createCollection(name.trim())).unwrap();
      setName("");
      onCreated?.();
    } catch (e) {
      setErr(typeof e === "string" ? e : "Failed to create collection");
    } finally {
      setBusy(false);
    }
  };
